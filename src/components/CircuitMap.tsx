...
export function getCircuitPointAtProgress(progress: number) {
  ...
  const len = p * TOTAL_LENGTH;
  return circuitProps.getPointAtLength(len);
}

export function getCircuitTangentAtProgress(progress: number) {
  const p = Math.max(0, Math.min(progress, 1));
  const len = p * TOTAL_LENGTH;
  const delta = Math.max(1, TOTAL_LENGTH * 0.002);
  const prev = circuitProps.getPointAtLength(Math.max(0, len - delta));
  const next = circuitProps.getPointAtLength(Math.min(TOTAL_LENGTH, len + delta));
